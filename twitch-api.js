...
class TwitchApi {
  ...
  static handleApiError(err) {
    ...
    if (res.data && res.data.message) {
      console.error('[TwitchApi]', 'API request fallo con Helix error:', res.data.message, `(${res.data.error}/${res.data.status})`);
    } else {
      console.error('[TwitchApi]', 'API request fallo con error:', err.message || err);
    }
  }
